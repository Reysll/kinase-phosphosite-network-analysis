...
def load_phospho():
    df = pd.read_excel(DATA_PATH, sheet_name="Phosphorylation", header=1)
...
def plot_hist(control_corr, case_corr):
    ...
    plt.ylabel("Density")
    plt.title("Phosphosite Correlation Distribution")
    plt.legend()
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "phospho_corr_control_vs_case.png", dpi=300)
    plt.close()


def main():
    df, control_cols, case_cols = load_phospho()

    df_control = filter_sites(df, control_cols)
    df_case = filter_sites(df, case_cols)

    print("Sites in control:", len(df_control))
    print("Sites in case:", len(df_case))
